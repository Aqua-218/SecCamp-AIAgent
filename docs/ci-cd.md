...
| System verification | `Direct Firecracker guest-control verification`, `Real Runtime lifecycle verification`, `Pinned Firecracker Runtime version matrix`, `Full SessionOwner KVM verification`, `SessionOwner external-crash recovery matrix`, `Authenticated systemd control-plane verification`, `Concurrent SessionOwner KVM verification`, `Exact installed production-chain verification`, `Real CapFS FUSE verification`, `Real public HTTPS verification`, `Privileged runtime isolation verification`, `Privileged aarch64 runtime isolation verification`, `Privileged runtime isolation soak` | `real_vm`, `real_runtime_lifecycle`, `real_runtime_version_matrix`, `session_owner_kvm`, `session_owner_crash_recovery`, `systemd_control_plane`, `concurrent_session_owners`, `installed_production_chain`, `real_capfs`, `egress_real_https`, `privileged_isolation`, `privileged_isolation_aarch64`, `runtime_isolation_soak` | Scheduled or protected on demand, on hosts that provide KVM, systemd/polkit, FUSE, root, namespaces, cgroup-v2, and the required kernel devices. See [Privileged runners](#privileged-runners) |
...
|---|---|---|
| `kvm` | root, systemd as PID 1, polkit, `/dev/kvm`, `/dev/vhost-vsock`, cgroup-v2, `veritysetup`, `busybox`, `mksquashfs`, `mkfs.ext4`, `truncate`, `tar`, `sha256sum` | Direct Firecracker guest-control, two-release Runtime matrix, full and two-owner SessionOwner KVM verification, authenticated systemd control plane, exact installed production chain, and 11-point crash recovery |
