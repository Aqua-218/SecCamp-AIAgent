...
| `runtime-isolation` | [runtime-isolation](runtime-isolation/README.md) | exec 直前の 13 step。namespace、mount、cgroup、Landlock、capability、seccomp | mock backend、特権hostの実syscall／production launcher post-exec probe、KVM guestのrootfs `/`、実rollback |
| `supervisor` | [Supervisor adapter](supervisor/README.md) | 認証済み connection の subject binding、wire protocol、subject lifecycle | local／実kernel resource gateとKVM guest supervisor、isolation launcher、全13 CapFS effectを確認 |
| `session-orchestrator` | [Session orchestrator](session-orchestrator/README.md) | session identity、backend lease、startup / rollback / stop | production adapter compositionと実KVM SessionOwner lifecycle（全13 CapFS effect、Broker、cleanup） |
