...
| モジュール | 現在の責務 |
| --- | --- |
| `transport` | `AF_VSOCK` listener 抽象、1 MiB 上限を検査してから payload を確保する frame I/O |
| `dispatch` | canonical CBOR 復元、replay cache、budget 予約、Capability の commit 境界、型付き adapter 呼び出し |
...
詳細は [GitHub 型付き adapter](github.md)、[公開 HTTPS policy](network-policy.md)、[transport 契約](transport.md)、[検証対応表](verification.md) を参照する。
