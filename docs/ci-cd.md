...
Promoting a gate is one field change plus the job it now demands. The manifest now has 44 implemented gates and no planned gates. The implemented deep gates use one pinned toolchain, execute non-empty supported subsets, and fail closed when a selection, baseline, corpus, tool, protected runner, or kernel device is missing. The `session_owner_kvm` gate drives the production composition from clean snapshot capture through all 13 CapFS effects, durable Broker evidence, stop, and exact resource cleanup. Production-launcher post-exec enforcement is also covered independently by the `privileged_isolation` gate's three real scenarios. The benchmark gate deliberately measures the pure capability decision path; `/dev/fuse` availability is not converted into a fake green result.
...
|---|---|---|
| `kvm` | root, `/dev/kvm`, `/dev/vhost-vsock`, cgroup-v2, `veritysetup`, `busybox`, `mksquashfs`, `mkfs.ext4`, `truncate`, `tar`, `sha256sum` | Direct Firecracker guest-control, Real Runtime lifecycle, and full SessionOwner KVM verification |
