...
このうち 4 byte big-endian の length prefix と 1 MiB の allocation 前上限、session ID、sequence、request ID、SHA-256 payload hash、bounded deduplication table は [`egress-protocol`](../../crates/egress-protocol/src/) に実装済みである。新しい envelope は一度だけ dispatch でき、完全に同じ retry は `Duplicate` として保存済み outcome を返す。session 不一致、順序違反、同じ request ID の別 payload、capacity / sequence exhaustion は dispatch 前に拒否する。さらに Broker の内部操作型は `HttpFetchRequest` または `GitHubRequest` の closed union であり、生の URL、HTTP method、header、body、credential を dispatch API に渡す variant はない。[canonical CBOR decoder](../egress-protocol/canonical-cbor.md)は fixed schema、definite length、最短 integer / length encoding、payload hash 一致を強制する。vsock/Firecracker UDS I/O、response cache、typed provider dispatch は sibling crate [`egress-broker`](../../crates/egress-broker/src/) が実装する。
