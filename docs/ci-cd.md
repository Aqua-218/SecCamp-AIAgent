...
The release boundary is deliberately limited to the artifact this repository can prove today: the `authority-corpus` Linux binary. The repository contains the multi-session controller, one-session worker/recovery systemd units, environment templates, polkit/udev policy, and deployment runbook, but no automated production rollout job, immutable service image, or repository-owned infrastructure target is present.
...
| Repository invariants | `Toolchain consistency`, `Declared dependency usage`, `Lockfile integrity`, `Documentation policy`, `Repository hygiene`, `Privileged service boundaries`, `Pin inventory` | `toolchain_consistency`, `dependency_usage`, `lockfile_integrity`, `docs_policy`, `repository_hygiene`, `service_boundaries`, `pin_inventory` | Pinned toolchains agree, every declared dependency is used, the lockfile matches the manifests, the documentation tree obeys its conventions, deployable controller/worker/polkit/device/environment contracts agree, no tracked file is an unreviewable blob or stray build output, and every pin is collected into one reviewable list |
...
| System verification | `Direct Firecracker guest-control verification`, `Real Runtime lifecycle verification`, `Pinned Firecracker Runtime version matrix`, `Full SessionOwner KVM verification`, `SessionOwner external-crash recovery matrix`, `Authenticated systemd control-plane verification`, `Concurrent SessionOwner KVM verification`, `Real CapFS FUSE verification`, `Real public HTTPS verification`, `Privileged runtime isolation verification`, `Privileged aarch64 runtime isolation verification`, `Privileged runtime isolation soak` | `real_vm`, `real_runtime_lifecycle`, `real_runtime_version_matrix`, `session_owner_kvm`, `session_owner_crash_recovery`, `systemd_control_plane`, `concurrent_session_owners`, `real_capfs`, `egress_real_https`, `privileged_isolation`, `privileged_isolation_aarch64`, `runtime_isolation_soak` | Scheduled or protected on demand, on hosts that provide KVM, systemd/polkit, FUSE, root, namespaces, cgroup-v2, and the required kernel devices. See [Privileged runners](#privileged-runners) |
| Protected external provider | `Protected external provider smoke` | `external_provider_smoke` | Runs only as a protected on-demand operation against an explicitly acknowledged disposable repository; scheduled pipelines skip it, and missing credentials or identity bindings fail before any request |
| Independent review | `Independent review attestation` | `external_review_attestation` | Protected on-demand import of a repository-external Ed25519-signed report bound to the exact commit/tree and a separately pinned reviewer key; automation validates evidence but does not create independence |
...
Promoting a gate is one field change plus the job it now demands. The manifest now has 52 implemented gates and no planned gates. The fast `service_boundaries` gate runs on every pipeline and locks the exact production service, polkit, udev, and environment contracts. The implemented deep gates use one pinned toolchain, execute non-empty supported subsets, and fail closed when a selection, baseline, corpus, tool, protected runner, or kernel device is missing. The `session_owner_kvm` gate drives the production composition from clean snapshot capture through all 13 CapFS effects, durable Broker evidence, stop, and exact resource cleanup; its separate crash matrix kills all 11 declared durable boundaries. `systemd_control_plane` exercises the authenticated non-privileged controller, forces a worker into `failed`, requires exact recovery before quota release, and repeats controller-crash reconciliation. `concurrent_session_owners` keeps two production KVM and Broker workers live after durable final responses and proves independent cleanup. These systemd-fixture and production-KVM gates remain separate executions; they do not claim an exact installed production-unit-to-VM end-to-end run. Production-launcher post-exec enforcement and the high-risk raw-syscall corpus are also covered independently by the `privileged_isolation` gate's four real scenarios and bounded 20-iteration soak. The benchmark gate deliberately measures the pure capability decision path; `/dev/fuse` availability is not converted into a fake green result.
...
The protected system-stage gates need hosts that hosted runner pools do not offer. They run on a schedule or on demand, never on pull requests, because a host carrying either label runs repository code with elevated rights, and because a gate that silently skips when a device is missing teaches reviewers to ignore it. The external-provider and independent-review gates are exceptions to the schedule rule: they are eligible only for protected on-demand dispatches because one mutates a disposable repository and the other requires revision-specific evidence from an external actor.

| Label | Host must provide | Gate |
|---|---|---|
| `kvm` | root, systemd as PID 1, polkit, `/dev/kvm`, `/dev/vhost-vsock`, cgroup-v2, `veritysetup`, `busybox`, `mksquashfs`, `mkfs.ext4`, `truncate`, `tar`, `sha256sum` | Direct Firecracker guest-control, two-release Runtime matrix, full and two-owner SessionOwner KVM verification, authenticated systemd control plane, and 11-point crash recovery |
| `privileged` | root, a readable/writable `/dev/fuse`, `dnsmasq`, `ip`, `mount`, `unshare`, `openssl`, and a cgroup v2 root delegating `memory` and `pids` to its children | Real CapFS FUSE, Real public HTTPS, Privileged runtime isolation verification and soak |
| `arm64` + `privileged` | Real aarch64 kernel plus the same namespace, Landlock, seccomp, AF_VSOCK, and delegated cgroup-v2 prerequisites | Privileged aarch64 runtime isolation verification |
| `external` | Protected runner, Cargo toolchain, GitHub App installation token, disposable repository, branch/object identities, and exact acknowledgement string | Protected external provider smoke |
| protected review environment | External report/signature/public key, separately configured key fingerprint, OpenSSL, and an exact clean checkout | Independent review attestation |

GitHub system-label names are declared in [`.github/actionlint.yaml`](../.github/actionlint.yaml) so the linter can tell a typo from a custom label; GitLab uses `kvm`, `privileged`, `external`, and `ephemeral` as runner tags. Neither script reports a skip as a pass: on a host missing the prerequisites they print what is absent and exit with a distinct status.

The protected external-provider environment or GitLab protected variables must provide exactly `EGRESS_GITHUB_TOKEN`, `EGRESS_GITHUB_INSTALLATION_ID`, `EGRESS_GITHUB_DISPOSABLE_REPOSITORY`, `EGRESS_GITHUB_BASE_BRANCH`, `EGRESS_GITHUB_HEAD_BRANCH`, `EGRESS_GITHUB_EXPECTED_OLD_OBJECT`, `EGRESS_GITHUB_NEW_OBJECT`, and `EGRESS_GITHUB_DISPOSABLE_ACK`. The acknowledgement must equal `I_UNDERSTAND_DISPOSABLE_REPOSITORY:<owner>/<name>` for the disposable repository; missing or mismatched values fail before the provider test sends a request.

## Triggers and cancellation

- GitHub runs CI and security on pull requests and `main`; CI also runs on `release/**`. Security runs weekly. Deep verification runs daily at 03:17 UTC and on demand; the destructive external-provider smoke is skipped on the daily schedule and is required only from protected `workflow_dispatch`. Tags matching `v*.*.*` enter the release workflow, where a strict semantic-version check rejects invalid tags.
