...
| System verification | `Direct Firecracker guest-control verification`, `Real Runtime lifecycle verification`, `Real CapFS FUSE verification`, `Real public HTTPS verification`, `Privileged runtime isolation verification` | `real_vm`, `real_runtime_lifecycle`, `real_capfs`, `egress_real_https`, `privileged_isolation` | Scheduled or protected on demand, on hosts that provide KVM, FUSE, root, namespaces, cgroup-v2, and the required kernel devices. See [Privileged runners](#privileged-runners) |
...
Promoting a gate is one field change plus the job it now demands. The manifest now has 43 implemented gates and one planned gate. The implemented deep gates use one pinned toolchain, execute non-empty supported subsets, and fail closed when a selection, baseline, corpus, tool, protected runner, or kernel device is missing. The remaining planned entry is a visible topology claim with no platform job: full SessionOwner KVM lifecycle. Production-launcher post-exec enforcement is covered by the `privileged_isolation` gate's three real scenarios, so it is not represented as a duplicate planned job. The benchmark gate deliberately measures the pure capability decision path; `/dev/fuse` availability is not converted into a fake green result.
...
The protected system-stage gates need hosts that hosted runner pools do not offer. They run on a schedule or on demand, never on pull requests, because a host carrying either label runs repository code with elevated rights, and because a gate that silently skips when a device is missing teaches reviewers to ignore it. The external-provider gate is the exception to the schedule rule: it is eligible only for protected on-demand dispatches because it mutates a disposable repository and requires operator-owned cleanup.

| Label | Host must provide | Gate |
|---|---|---|
| `kvm` | root, `/dev/kvm`, `/dev/vhost-vsock`, cgroup-v2, `veritysetup`, `busybox`, `mksquashfs`, `mkfs.ext4`, `truncate`, `tar`, `sha256sum` | Direct Firecracker guest-control and Real Runtime lifecycle verification |
| `privileged` | root, a readable/writable `/dev/fuse`, `dnsmasq`, `ip`, `mount`, `unshare`, `openssl`, and a cgroup v2 root delegating `memory` and `pids` to its children | Real CapFS FUSE, Real public HTTPS, Privileged runtime isolation verification |
