...
未実装なのは、File以外のauthority variant、durable audit backend、supervisor / Broker adapter、FUSEのcreate・remove・rename・metadata変更である。Direct-I/O [`capfs` adapter](../capfs/read-only-fuse.md)がglobal namespace registry、Authority handle registry、実backing syscallを接続し、実mount上のread-after-revoke、write-after-revoke、readdir-after-revokeまで検査している。truncate、rename、unlinkを含むtransactionと競合検証はまだない。
