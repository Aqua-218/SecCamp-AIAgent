...
- production の caller resolver は未実装に近い。`StaticCallerResolver` を使う test はあるが、実 socket の peer credential から subject を解決する経路は検証していない。実 connection と実 credential は [Supervisor adapter](../supervisor/README.md) の未検証範囲にある。
