...
| System verification | `Direct Firecracker guest-control verification`, `Real Runtime lifecycle verification`, `Pinned Firecracker Runtime version matrix`, `SessionOwner external-crash recovery matrix`, `Real CapFS FUSE verification`, `Real public HTTPS verification`, `Privileged runtime isolation verification`, `Privileged aarch64 runtime isolation verification`, `Privileged runtime isolation soak` | `real_vm`, `real_runtime_lifecycle`, `real_runtime_version_matrix`, `session_owner_crash_recovery`, `real_capfs`, `egress_real_https`, `privileged_isolation`, `privileged_isolation_aarch64`, `runtime_isolation_soak` | Scheduled or protected on demand, on hosts that provide KVM, FUSE, root, namespaces, cgroup-v2, and the required kernel devices. See [Privileged runners](#privileged-runners) |
...
Promoting a gate is one field change plus the job it now demands. The manifest now has 49 implemented gates and no planned gates. The implemented deep gates use one pinned toolchain, execute non-empty supported subsets, and fail closed when a selection, baseline, corpus, tool, protected runner, or kernel device is missing. The `session_owner_kvm` gate drives the production composition from clean snapshot capture through all 13 CapFS effects, durable Broker evidence, stop, and exact resource cleanup; its separate crash matrix kills all 11 declared durable boundaries. Production-launcher post-exec enforcement and the high-risk raw-syscall corpus are also covered independently by the `privileged_isolation` gate's four real scenarios and bounded 20-iteration soak. The benchmark gate deliberately measures the pure capability decision path; `/dev/fuse` availability is not converted into a fake green result.
...
|---|---|---|
| `kvm` | root, `/dev/kvm`, `/dev/vhost-vsock`, cgroup-v2, `veritysetup`, `busybox`, `mksquashfs`, `mkfs.ext4`, `truncate`, `tar`, `sha256sum` | Direct Firecracker guest-control, two-release Runtime matrix, full SessionOwner KVM verification, and 11-point crash recovery |
