...
- Capability の subject binding、保持、発行、祖先失効、revoke を含む Rust 状態機械全体の安全性が、数学的に証明済みであるとは言えない。逐次 transition は実装され、参照モデルとの property test まであるが、Lean の定理にはなっていない。revoke と外部 effect の並行実行もまだ対象外である。
